...
def test_sidebar_controls_render():
    ...
    assert any("Window Length" in lbl for lbl in widget_labels)
    assert any("Refresh Interval" in lbl for lbl in widget_labels)
    assert any("Protocol Filter" in lbl for lbl in widget_labels)
    assert any("Top-N" in lbl for lbl in widget_labels)
